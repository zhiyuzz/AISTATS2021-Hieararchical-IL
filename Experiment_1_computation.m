%% Experiment 1
% Evaluate the effect of T on the optimization of theta
...
global parameter
parameter = NaN(K,length(Set_length),N+1,3);

%% Main loop

Index_sample_path = 1:K;
Index_length = 1:length(Set_length);
...
parfor index_grid = 1:numel(G1)
    ...
    T = Set_length(G2(index_grid)); %#ok<PFBNS>
    seq = obs(1:T,:);
...
save('Experiment_1_output.mat','parameter')
